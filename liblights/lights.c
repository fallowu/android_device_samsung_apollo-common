...
static int
write_string(char const* path, char const *value) {
    int fd;
    static int already_warned = 0;
...
set_light_backlight(struct light_device_t* dev,
    ...
    property_get("persist.sys.led_trigger", def_mode, "heartbeat");
	
    err = write_int(LCD_FILE, brightness);

    is_scr_on = brightness > 0 ? 1 : 0;
    err = write_string(TRIGGER_FILE, is_scr_on ? "none" : def_mode);
...
set_light_buttons(struct light_device_t* dev,
    ...
    err = write_string(TRIGGER_FILE, on ? "default-on" : is_scr_on ? "none" : def_mode);
...
set_light_notification(struct light_device_t* dev,
    ...
    if (v > 0) {
       err = write_string(TRIGGER_FILE, "timer");
       err = write_int(DELAY_ON_FILE, 1000);
       err = write_int(DELAY_OFF_FILE, 1000);
    }
    else {
       err = write_string(TRIGGER_FILE, is_scr_on ? "none" : def_mode );
    }
