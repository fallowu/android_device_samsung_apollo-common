...
#define INPUT_SOURCE_KEY "input_source"
...
AudioHardware::AudioHardware() :
    ...
    mDriverOp(DRV_NONE)
{
    RilOnComplete onRawReqComplete;
    RilOnUnsolicited onUnsol;
#if 0
    ...
    loadRILD();
    if (registerRequestCompleteHandler(mRilClient, 59, onRawReqComplete) != RIL_CLIENT_ERR_SUCCESS) {
        LOGE("[*] RegisterRequestCompleteHandler() error/n");
    }

    if (registerUnsolicitedHandler(mRilClient, 11004, onUnsol) != RIL_CLIENT_ERR_SUCCESS){
...
void AudioHardware::loadRILD(void)
{
    ...
    if (mSecRilLibHandle) {
        ...
        registerUnsolicitedHandler  = (int (*)(HRilClient , uint32_t , RilOnUnsolicited ))
                              dlsym(mSecRilLibHandle, "RegisterUnsolicitedHandler");
        registerRequestCompleteHandler      = (int (*)(HRilClient , uint32_t , RilOnComplete ))
                              dlsym(mSecRilLibHandle, "RegisterRequestCompleteHandler");
        invokeOemRequestHookRaw      = (int (*)(HRilClient , char *, size_t ))
...
int AudioHardware::convertSoundType(SoundType type) {
    switch (type) {
        ...
        default:
            LOGE("convertSoundType: unknown type! %d", type);
...
int AudioHardware::convertAudioPath(AudioPath path) {
    switch (path) {
        ...
        default:
            LOGE("convertAudioPath: unknown path! %d", path);
...
status_t AudioHardware::setVoiceVolume_l(float volume)
{
    LOGI("### setVoiceVolume_l");
    ...
    if ( (AudioSystem::MODE_IN_CALL == mMode) && (mSecRilLibHandle) &&
        ...
        uint32_t routes = AudioSystem::DEVICE_OUT_EARPIECE;
        if (mOutput != NULL) {
...
status_t AudioHardware::setMode(int mode)
{
    ...
    if (status == NO_ERROR) {
        ...
        if (mMode == AudioSystem::MODE_NORMAL) {
            if(mActivatedCP) {
	        setCallClockSync(mRilClient, SOUND_CLOCK_STOP);
                mActivatedCP = false;
            }
        }
...
status_t AudioHardware::doRouting_l(uint32_t device, bool force)
{
...
status_t AudioHardware::setIncallPath_l(uint32_t device)
{
    ...
    return doRouting_l(device, true);
}
...
status_t AudioHardware::setVoiceMemoPath_l(String8 path)
{
    LOGI("setVoiceMemoPath_l() path %s", path.string());
...
status_t AudioHardware::AudioStreamInALSA::open_l()
{
	...
	if (mHardware->mode() == AudioSystem::MODE_IN_CALL) {
		...
		if (ctlMic != NULL ) {
		    // set proper Mic Path router for voicecall.
		    TRACE_DRIVER_IN(DRV_MIXER_SEL)
	...
	if (mHardware->mode() != AudioSystem::MODE_IN_CALL) {
		...
		if (ctlMic != NULL ) {
		    // set proper Mic Path router for voicememo
		    TRACE_DRIVER_IN(DRV_MIXER_SEL)
		    mixer_ctl_select(ctlMic , mHardware->getInputRouteFromDevice(mDevices));
		    TRACE_DRIVER_OUT
		}
