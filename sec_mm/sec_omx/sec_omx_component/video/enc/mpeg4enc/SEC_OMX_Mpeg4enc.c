...
#if 1
    SEC_OSAL_Log(SEC_LOG_TRACE, "allocate input buffer\n");
    /* allocate encoder's input buffer */
    returnCodec = SsbSipMfcEncGetInBuf(hMFCHandle, &inputInfo);
    if (returnCodec != MFC_RET_OK) {
        ret = OMX_ErrorInsufficientResources;
        goto EXIT;
    }
#endif
    SEC_OSAL_Log(SEC_LOG_TRACE, "process data\n");
    SEC_OSAL_Log(SEC_LOG_TRACE, "process data : INPUT_PORT_INDEX = %d\n", INPUT_PORT_INDEX);
    SEC_OSAL_Log(SEC_LOG_TRACE, "process data : YPhyAddr = %x\n", inputInfo.YPhyAddr);
    SEC_OSAL_Log(SEC_LOG_TRACE, "process data : CPhyAddr = %x\n", inputInfo.CPhyAddr);
    SEC_OSAL_Log(SEC_LOG_TRACE, "process data : YVirAddr = %x\n", inputInfo.YVirAddr);
    SEC_OSAL_Log(SEC_LOG_TRACE, "process data : CVirAddr = %x\n", inputInfo.CVirAddr);
    SEC_OSAL_Log(SEC_LOG_TRACE, "process data : YSize = %x\n", inputInfo.YSize);
    SEC_OSAL_Log(SEC_LOG_TRACE, "process data : CSize = %x\n", inputInfo.CSize);
    SEC_OSAL_Log(SEC_LOG_TRACE, "before memset\n");
    SEC_OSAL_Log(SEC_LOG_TRACE, "memset\n");
...
OMX_ERRORTYPE SEC_MFC_Mpeg4_Encode(OMX_COMPONENTTYPE *pOMXComponent, SEC_OMX_DATA *pInputData, SEC_OMX_DATA *pOutputData)
{
    ...
    SSBSIP_MFC_ENC_INPUT_INFO *pInputInfo = &(inputInfo);
