...
SSBSIP_MFC_ERROR_CODE SsbSipMfcEncGetInBuf(void *openHandle, SSBSIP_MFC_ENC_INPUT_INFO *input_info)
{
    ...
    LOGD("SsbSipMfcEncGetInBuf : enter ++");
    if (openHandle == NULL) {
    ...
    pCTX = (_MFCLIB *)openHandle;
    LOGD("SsbSipMfcEncGetInBuf : get y_size, c_size : width = %d height = %d", pCTX->width, pCTX->height);
    ...
    LOGD("SsbSipMfcEncGetInBuf : Allocate luma & chroma buf");
    ...
    LOGD("SsbSipMfcEncGetInBuf : Get physical address information");
    ...
    LOGD("SsbSipMfcEncGetInBuf : input info");
    LOGD("SsbSipMfcEncGetInBuf : input_info->YPhyAddr = %x", pCTX->phyFrmBuf.luma);
    LOGD("SsbSipMfcEncGetInBuf : input_info->CPhyAddr = %x", pCTX->phyFrmBuf.chroma);
    LOGD("SsbSipMfcEncGetInBuf : input_info->YVirAddr = %x", pCTX->virFrmBuf.luma);
    LOGD("SsbSipMfcEncGetInBuf : input_info->CVirAddr = %x", pCTX->virFrmBuf.chroma);

    LOGD("input_info : %x", input_info);
    ...
    LOGD("SsbSipMfcEncGetInBuf 2: input info");
    LOGD("SsbSipMfcEncGetInBuf 2: input_info->YPhyAddr = %x", input_info->YPhyAddr);
    LOGD("SsbSipMfcEncGetInBuf 2: input_info->CPhyAddr = %x", input_info->CPhyAddr);
    LOGD("SsbSipMfcEncGetInBuf 2: input_info->YVirAddr = %x", input_info->YVirAddr);
    LOGD("SsbSipMfcEncGetInBuf 2: input_info->CVirAddr = %x", input_info->CVirAddr);

    LOGD("SsbSipMfcEncGetInBuf : exit --");
    return MFC_RET_OK;
}
